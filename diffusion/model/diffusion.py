...
class GeneralContinuousTimeDiffusion(ModelMixin, ConfigMixin):
    ...
    def from_config(cls, cfg: DictConfig) -> dict[str, Any]:
        return {
            ...
            "require_processing": cfg.MODEL.REQUIRE_PROCESSING,
        }
    ...
    def forward(self,
        ...
        if self.require_processing:
            ...
            return c_skip * x + c_out * self.backbone(c_in * x, scale, c_noise, condition)
        else:
    ...
    def pre_and_post_processing(self, scale: torch.Tensor, sigma: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.prediction_type == "sample":
            ...
            c_skip = scale * self.sigma_data ** 2 / ((scale * self.sigma_data) ** 2 + sigma ** 2)
